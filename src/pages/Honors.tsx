...
export function Honors() {
  ...
  const handleHonorClick = (index: number) => {
    setActiveHonor(activeHonor === index ? null : index);
  };
  ...
  return (
    <section id="honors" className="pt-8 md:pt-16 min-h-screen relative overflow-hidden">
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
            ...
            {honors.map((honor, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div className={`relative transition-all duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] ${isActive ? 'h-64 sm:h-80' : 'h-28'}`}>
                    <img
                      alt={honor.title}
                      src={honor.image}
                  ...
                  {/* Expanded Content Area (Grid Reveal) */}
                  <div
                  ...
                  >
                    <div className="overflow-hidden min-h-0">
                      <div className="px-6 pb-8 pt-4">
                        <span className="font-label text-[10px] tracking-[0.2em] font-bold uppercase text-primary mb-2 block">{honor.subtitle}</span>
